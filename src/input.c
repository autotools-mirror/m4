...
/* TRUE iff strlen(m4_rquote) == strlen(m4_lquote) == 1 */
static boolean single_quotes;

/* TRUE iff strlen(m4_bcomm) == strlen(m4_ecomm) == 1 */
...
void
push_macro (m4_builtin_func *func, boolean traced)
{
  input_block *i;
...
input_init (void)
{
  ...
  m4_lquote.string = xstrdup (M4_DEF_LQUOTE);
  m4_lquote.length = strlen (m4_lquote.string);
  m4_rquote.string = xstrdup (M4_DEF_RQUOTE);
  m4_rquote.length = strlen (m4_rquote.string);
  single_quotes = TRUE;

  m4_bcomm.string = xstrdup (M4_DEF_BCOMM);
  m4_bcomm.length = strlen (m4_bcomm.string);
  m4_ecomm.string = xstrdup (M4_DEF_ECOMM);
  m4_ecomm.length = strlen (m4_ecomm.string);
...
syntax_init (void)
{
  ...
  set_syntax_internal(M4_SYNTAX_LQUOTE, m4_lquote.string[0]);
  set_syntax_internal(M4_SYNTAX_RQUOTE, m4_rquote.string[0]);
  set_syntax_internal(M4_SYNTAX_BCOMM, m4_bcomm.string[0]);
  set_syntax_internal(M4_SYNTAX_ECOMM, m4_ecomm.string[0]);
}
...
set_quotes (const char *lq, const char *rq)
{
  ...
  xfree (m4_lquote.string);
  xfree (m4_rquote.string);

  m4_lquote.string = xstrdup (lq ? lq : M4_DEF_LQUOTE);
  m4_lquote.length = strlen (m4_lquote.string);
  m4_rquote.string = xstrdup (rq ? rq : M4_DEF_RQUOTE);
  m4_rquote.length = strlen (m4_rquote.string);

  single_quotes = (m4_lquote.length == 1 && m4_rquote.length == 1);
...
set_comment (const char *bc, const char *ec)
{
  ...
  xfree (m4_bcomm.string);
  xfree (m4_ecomm.string);

  m4_bcomm.string = xstrdup (bc ? bc : M4_DEF_BCOMM);
  m4_bcomm.length = strlen (m4_bcomm.string);
  m4_ecomm.string = xstrdup (ec ? ec : M4_DEF_ECOMM);
  m4_ecomm.length = strlen (m4_ecomm.string);

  single_comments = (m4_bcomm.length == 1 && m4_ecomm.length == 1);
...
void
set_syntax (char key, const unsigned char *chars)
{
  int ch, code;

  code = syntax_code (key);
...
next_token (m4_token_data *td)
{
  ...
  do {
    ...
    else if (!single_comments && MATCH (ch, m4_bcomm.string))
      {
	obstack_grow (&token_stack, m4_bcomm.string, m4_bcomm.length);
	while ((ch = next_char ()) != CHAR_EOF && !MATCH (ch, m4_ecomm.string))
	  obstack_1grow (&token_stack, ch);
	if (ch != CHAR_EOF)
	  obstack_grow (&token_stack, m4_ecomm.string, m4_ecomm.length);
	type = discard_comments ? M4_TOKEN_NONE : M4_TOKEN_STRING;
      }
    ...
    else if (!single_quotes && MATCH (ch, m4_lquote.string))
      {
	quote_level = 1;
	while (1)
	  {
	    ...
	    if (MATCH (ch, m4_rquote.string))
	      {
		if (--quote_level == 0)
		  break;
		obstack_grow (&token_stack, m4_rquote.string, m4_rquote.length);
	      }
	    else if (MATCH (ch, m4_lquote.string))
	      {
		quote_level++;
		obstack_grow (&token_stack, m4_lquote.string, m4_lquote.length);
	      }
	    else
	      obstack_1grow (&token_stack, ch);
	  }
...
int
print_token (const char *s, m4_token_t t, m4_token_data *td)
{
  fprintf (stderr, "%s: ", s);
  switch (t)
    {				/* TOKSW */
